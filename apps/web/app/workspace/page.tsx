...
export default async function WorkspacePage() {
  ...
  return (
    <main className="workspace-shell">
      ...
      {!!experiments?.length && <section className="imported-experiments"><p className="eyebrow">Imported evidence</p>{experiments.map((experiment) => <article key={experiment.id}><h2>{experiment.name}</h2><div>{experiment.runs.map((run) => <span key={run.id}>{run.name}</span>)}</div></article>)}</section>}
    </main>
  );
}
